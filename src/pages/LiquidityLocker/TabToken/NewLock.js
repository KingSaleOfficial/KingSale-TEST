...
function Roadmap(props) {
  ...
  const addTokenLockerHandler = async () => {
    try {
      const weiLockPrice1 =
        (parseInt(formData.lockAmount) / 100) * parseFloat(totalSupply);
      const res = await axios.post(
        apiConfig.addTokenLocker,
        {
  ...
  return (
    <Box pt={2} pb={6}>
      <Box className={classes.mainBox}>
        <Box className={classes.connectCard}></Box>

        <Grid spacing={1} container>
          <Grid item xs={12} sm={12}>
            <Box mt={2}>
              <label>
                Enter the token address you would like to lock token for
              </label>
              <TextField
        ...
        <Box mt={2}>
          <label>Unlock Amount ({Number(lockPriceByPer).toFixed(2)})</label>
          {/* <TextField
            disabled={isUpdating}
            id="outlined-basic"
            variant="outlined"
        ...
        <Box align='center' mt={3}>
          ...
          <Box textAlign='left' mt={1} p={1}>
            <Typography>
              *Please exclude KingLock’s lockup address
              0x7ee058420e5937496F5a2096f04caA7721cF70cc from fees, rewards, max
              tx amount to start locking tokens.
            </Typography>
          </Box>
        </Box>
      </Box>
